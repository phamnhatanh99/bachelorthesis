...
import rptu.thesis.npham.dsserver.service.Profiler;
...
public class Test {
    ...
    @Autowired
    public Test(MetadataRepo metadata_repository, SketchesRepo sketches_repository, SimilarityScoresRepo similarity_scores_repository, Profiler profiler, SimilarityCalculator similarity_calculator, LSHIndex LSHIndex) {
    ...
    public String importAll() {
        ...
        sketches.forEach(LSHIndex::updateIndex);
...
//        metadatas.forEach(this::storeSimilarityScores);
    ...
    private void storeSimilarityScores(Metadata metadata) {
        ...
        for (Metadata m: containment_candidate) {
            ...
            Measure table_name_measure = new Measure(MeasureType.TABLE_NAME_WORDNET, table_name_sim, 1);
            double column_name_sim = column_name_similarity.get(m);
            Measure column_name_measure = new Measure(MeasureType.COLUMN_NAME_WORDNET, column_name_sim, 1);
            double containment_sim = containment_similarity.get(m).jcx();
            Measure containment_measure = new Measure(MeasureType.COLUMN_VALUE, containment_sim, 1);
            double format_sim = format_similarity.getOrDefault(m, new Jaccard(0, 0, 0)).jcx();
            Measure format_measure = new Measure(MeasureType.COLUMN_FORMAT, format_sim, 1);
            ...
            Measure containment_measure_candidate = new Measure(MeasureType.COLUMN_VALUE, containment_sim_candidate, 1);
            double format_sim_candidate = format_similarity.getOrDefault(m, new Jaccard(0, 0, 0)).jcy();
            Measure format_measure_candidate = new Measure(MeasureType.COLUMN_FORMAT, format_sim_candidate, 1);
            Measures measure_list_candidate = new Measures(new ArrayList<>());
    ...
    public String delete(@PathVariable String id) {
        ...
        LSHIndex.removeSketchFromIndex(id);
        return "Deleted " + id;
    }
}
